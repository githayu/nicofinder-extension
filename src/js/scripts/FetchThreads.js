...
class FetchThreads {
  ...
  createPacket() {
    ...
    if (this.hasDefaultThread) {
      if (this.isPlayer) {
        packet.push(this.createThread({
          thread: this.defaultThreadId
        }));
        packet.push(this.createThreadLeaves({
          thread: this.defaultThreadId
        }));
  ...
  createThreadLeaves(options) {
    ...
    const maxChat = this.videoDuration < 60 ? 100
      : this.videoDuration < 300 ? 250
        : this.videoDuration < 600 ? 500 : 1000;
